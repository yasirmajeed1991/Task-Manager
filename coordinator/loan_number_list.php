        ...
        <div class="card-body">
            ...
            <table class="table tabe-hover table-bordered" id="list">
                ...
                <tbody>
                    ...
                    SELECT * 
                    FROM loan_no
                    WHERE JSON_CONTAINS(coordinator, '\"{$_SESSION['login_id']}\"') 
                    ORDER BY id DESC
                    "); 
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <?php if ($scrub_result) {?>
                            <a class="dropdown-item" id="start_scrub"
                            href="javascript:void(0)" data-id="<?php echo  $row['id']; ?>" data-loanid="<?php echo  $row['id']; ?>" >Start Scrub</a>
                               
                                
                            <?php }else{?>
                            
                                <a class="dropdown-item"
                                    href="./index.php?page=edit_scrub&id=<?php  $edit_scrub = $conn->query("SELECT id FROM scrub WHERE loan_no_id='".$row['id']."'")->fetch_assoc();echo $edit_scrub['id']?>">View Scrub</a>
...
$(document).ready(function() {
    ...
    $('#list tbody').on('click', '.assign_borrower', function() {
        uni_modal("Assign Borrower:", "assign_borrower.php?id=" + $(this)
            .attr('data-id') + "&branch=" + $(this).attr('data-branch'), "mid-large")
    })
    ...
    $('#list tbody').on('click', '.delete_loan_number', function() {
        _conf("Deleting this Loan details will also delete scrubs Order Outs associated with this record! Are you sure to continue?",
