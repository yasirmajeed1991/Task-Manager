...
Class Action {
	...
	function save_loan_no(){
	    ...
	    if(empty($_POST['loan_no']) || empty($_POST['branch']) || empty($_POST['processor']) || empty($_POST['coordinator']) ){
			return 3;
			exit;
		...
		if(empty($id)){
			...
			if($save){
				$processor_name = $this->db->query("SELECT * FROM users WHERE id='".$processor."' ");
	...
	function save_over_due_interval(){
		...
		$check = $this->db->query("SELECT * FROM over_due WHERE branch = '".$branch."' ")->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
	...
	function delete_loan_number() {
		...
		$scrub_query = $this->db->query("SELECT id FROM scrub WHERE loan_no_id = ".$id);
		if($scrub_query->num_rows > 0){
			$scrub_id = $scrub_query->fetch_assoc()['id'];
			
			// Get the order_outs_id associated with the scrub_id
			$order_outs_query = $this->db->query("SELECT id FROM order_outs WHERE scrub_id = ".$scrub_id);
			$ids = array();
			if($order_outs_query->num_rows > 0){
				while ($row = $order_outs_query->fetch_assoc()) {
					$ids[] = $row['id'];
				}
				
				// Delete the order_out_follow_up record associated with the order_outs_id
				if (!empty($ids)) {
					$id_list = implode(',', $ids);
					$delete5 = $this->db->query("DELETE FROM follow_up WHERE order_out_id IN (".$id_list.")");
				}
				
				// Delete the order_outs record 
				$delete4 = $this->db->query("DELETE FROM order_outs WHERE scrub_id = ".$scrub_id);
			}
			
			// Delete the scrub_followup record
			$delete3 = $this->db->query("DELETE FROM scrub_followup WHERE scrub_id = ".$scrub_id);
			
			// Delete the scrub record
			$delete2 = $this->db->query("DELETE FROM scrub WHERE id = ".$scrub_id);
		}
	
		// Delete the borrower record
		$delete0 = $this->db->query("DELETE FROM borrower WHERE loan_no_id = ".$id);
	...
	function delete_odi(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM over_due where id = ".$id);
	...
	function delete_scrub(){
		extract($_POST);
		// Get the order_outs_id associated with the scrub_id
		$order_outs_query = $this->db->query("SELECT id FROM order_outs WHERE scrub_id = ".$id);
		$ids = array();
		if($order_outs_query->num_rows > 0){
			while ($row = $order_outs_query->fetch_assoc()) {
				$ids[] = $row['id'];
			...
			// Delete the order_outs record 
			$delete4 = $this->db->query("DELETE FROM order_outs WHERE scrub_id = ".$id);
		}
		...
		// Delete the scrub record
		$delete2 = $this->db->query("DELETE FROM scrub WHERE id = ".$id);
	
		if ($delete2 || $delete3 || $delete4 || $delete5) {
			return 1;
		}
	}
	...
	function delete_branch(){
		...
		// Get all loan_no_ids associated with the branch
		$loan_query = $this->db->query("SELECT id FROM loan_no WHERE branch = '".$id."'");
		$loan_ids = array();
		while ($row = $loan_query->fetch_assoc()) {
			$loan_ids[] = $row['id'];
		}
		
		// Delete all records associated with the loan_no_ids
		if (!empty($loan_ids)) {
			$id_list = implode(',', $loan_ids);
			// Delete the borrower record
			$delete0 = $this->db->query("DELETE FROM borrower WHERE loan_no_id IN (".$id_list.")");
			// Delete the scrub record
			$delete2 = $this->db->query("DELETE FROM scrub WHERE loan_no_id IN (".$id_list.")");
			// Delete the scrub_followup record
			$delete3 = $this->db->query("DELETE FROM scrub_followup WHERE scrub_id IN (SELECT id FROM scrub WHERE loan_no_id IN (".$id_list."))");
			// Delete the order_outs record 
			$delete4 = $this->db->query("DELETE FROM order_outs WHERE scrub_id IN (SELECT id FROM scrub WHERE loan_no_id IN (".$id_list."))");
			// Delete the order_out_follow_up record associated with the order_outs_id
			$delete5 = $this->db->query("DELETE FROM follow_up WHERE order_out_id IN (SELECT id FROM order_outs WHERE scrub_id IN (SELECT id FROM scrub WHERE loan_no_id IN (".$id_list."))))");
			// Delete the loan_no record
			$delete1 = $this->db->query("DELETE FROM loan_no WHERE branch = '".$id."'");
		} else {
			// Delete only the branch record
			$delete1 = $this->db->query("DELETE FROM branch WHERE id = ".$id);
		}
